module Evergreen.V45.Types exposing (..)

import Browser
import Browser.Navigation
import Dict
import Evergreen.V45.Api.Card
import Evergreen.V45.Api.User
import Evergreen.V45.Auth.Common
import Evergreen.V45.Bridge
import Evergreen.V45.Gen.Pages
import Evergreen.V45.Shared
import Lamdera
import Time
import Url


type alias FrontendModel =
    { url : Url.Url
    , key : Browser.Navigation.Key
    , shared : Evergreen.V45.Shared.Model
    , page : Evergreen.V45.Gen.Pages.Model
    , authFlow : Evergreen.V45.Auth.Common.Flow
    , authRedirectBaseUrl : Url.Url
    }


type alias Session =
    { userId : Int
    , expires : Time.Posix
    }


type alias BackendModel =
    { sessions : Dict.Dict Lamdera.SessionId Session
    , users : Dict.Dict Int Evergreen.V45.Api.User.UserFull
    , cards : Dict.Dict Evergreen.V45.Api.Card.CardId Evergreen.V45.Api.Card.CardEnvelope
    , now : Time.Posix
    , nextCardId : Int
    , pendingAuths : Dict.Dict Lamdera.SessionId Evergreen.V45.Auth.Common.PendingAuth
    }


type FrontendMsg
    = ChangedUrl Url.Url
    | ClickedLink Browser.UrlRequest
    | Shared Evergreen.V45.Shared.Msg
    | Page Evergreen.V45.Gen.Pages.Msg
    | Noop


type alias ToBackend =
    Evergreen.V45.Bridge.ToBackend


type BackendMsg
    = AuthBackendMsg Evergreen.V45.Auth.Common.BackendMsg
    | CheckSession Lamdera.SessionId Lamdera.ClientId
    | RenewSession Evergreen.V45.Api.User.UserId Lamdera.SessionId Lamdera.ClientId Time.Posix
    | NoOpBackendMsg
    | Tick Time.Posix


type ToFrontend
    = AuthToFrontend Evergreen.V45.Auth.Common.ToFrontend
    | ActiveSession Evergreen.V45.Api.User.User
    | PageMsg Evergreen.V45.Gen.Pages.Msg
    | NoOpToFrontend
