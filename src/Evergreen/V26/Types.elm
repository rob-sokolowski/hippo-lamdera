module Evergreen.V26.Types exposing (..)

import Browser
import Browser.Navigation
import Dict
import Evergreen.V26.Api.Card
import Evergreen.V26.Api.User
import Evergreen.V26.Auth.Common
import Evergreen.V26.Bridge
import Evergreen.V26.Gen.Pages
import Evergreen.V26.Shared
import Lamdera
import Time
import Url


type alias FrontendModel =
    { url : Url.Url
    , key : Browser.Navigation.Key
    , shared : Evergreen.V26.Shared.Model
    , page : Evergreen.V26.Gen.Pages.Model
    , authFlow : Evergreen.V26.Auth.Common.Flow
    , authRedirectBaseUrl : Url.Url
    }


type alias Session =
    { userId : Int
    , expires : Time.Posix
    }


type alias BackendModel =
    { sessions : Dict.Dict Lamdera.SessionId Session
    , users : Dict.Dict Int Evergreen.V26.Api.User.UserFull
    , cards : Dict.Dict Evergreen.V26.Api.Card.CardId Evergreen.V26.Api.Card.CardEnvelope
    , now : Time.Posix
    , nextCardId : Int
    , pendingAuths : Dict.Dict Lamdera.SessionId Evergreen.V26.Auth.Common.PendingAuth
    }


type FrontendMsg
    = ChangedUrl Url.Url
    | ClickedLink Browser.UrlRequest
    | Shared Evergreen.V26.Shared.Msg
    | Page Evergreen.V26.Gen.Pages.Msg
    | Noop


type alias ToBackend =
    Evergreen.V26.Bridge.ToBackend


type BackendMsg
    = AuthBackendMsg Evergreen.V26.Auth.Common.BackendMsg
    | CheckSession Lamdera.SessionId Lamdera.ClientId
    | RenewSession Evergreen.V26.Api.User.UserId Lamdera.SessionId Lamdera.ClientId Time.Posix
    | NoOpBackendMsg
    | Tick Time.Posix
    | IncrementCardId


type ToFrontend
    = AuthToFrontend Evergreen.V26.Auth.Common.ToFrontend
    | ActiveSession Evergreen.V26.Api.User.User
    | PageMsg Evergreen.V26.Gen.Pages.Msg
    | NoOpToFrontend
